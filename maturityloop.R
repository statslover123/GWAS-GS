...
training=-testing
myGAPIT=GAPIT(
  Y=trait22[training,c(1,2)],  SNP.MAF =.1,
  GD=gene22,
  GM=map22,
  PCA.total=3,
...
df2 <- df2 %>% 
  add_row(model = 'QTN5', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
myGAPIT3 <-GAPIT(
  Y=trait22[training,c(1,2)],
  CV=myQTN3, 
  model="GLM", 
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'QTN3', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'QTN1', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
myGAPIT=GAPIT(
  Y=trait22[training,c(1,2)], 
  GD=gene22,
  GM=map22,
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'gBLUP', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'gBLUP qtn1', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
myGAPITgblup3=GAPIT(
  Y=trait22[training,c(1,2)], 
  GD=gene22,
  GM=map22,
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'gBLUP qtn3', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'gBLUP qtn5', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'cblup', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
myGAPITcBLUP1=GAPIT(
  Y=trait22[training,c(1,2)], 
  GD=gene22,
  GM=map22,
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'cblup qtn1', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
myGAPITcBLUP3=GAPIT(
  Y=trait22[training,c(1,2)], 
  GD=gene22,
  GM=map22,
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'cblup qtn3', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
ff <- ff[match(trait22$Taxa, ff$Taxa), ] 
df2 <- df2 %>% 
  add_row(model = 'cblup qtn5', cor=cor(ff$Prediction[testing],ff$Maturity[testing]), rmse =  (mean(abs(ff$Prediction[testing]-ff$Maturity[testing]))))
...
write.csv(df2, 'Maturityloopsecond25.csv')